...
class TruncLcgRand(Rng):
  """A random number generator using a truncated LCG.

  This generator uses a truncated LCG (like gmp_randinit_lc_2exp() in GMP).
  ...
  We define several LCGs of different size. The LCG depends on the
  output size. The size of the state of the LCG is twice the size of its
  output. This is the case, since truncated LCG only outputs the upper half
  of its state.
  """

  def __init__(self, output_size: int):
    """Constructs a random number generator using a truncated LCG.

    Args:
      output_size: the output size of the LCG. The internal state of the LCG is
        2*output_size.
    """
    # Reference values of the multiplier, format: {<state size>: <multiplier>}.
    # Taken from Table 4 (except for the 256-bit state) in the paper:
    # L’Ecuyer, P. Tables of Linear Congruential Generators of Different Sizes
    # and Good Lattice Structure. Math. Comp. 1999, 68, 249–260,
    ...
    ref_multipliers = {
        ...
        40: 330169576829,
        48: 181465474592829,
        60: 454339144066433781,
        63: 9219741426499971445,
        64: 2862933555777941757,
        96: 75564983892026345434470042133,
        128: 47026247687942121848144207491837418733,
        # 256-bit multiplier is generated by using accompanying code for the
        # paper: Steele, G.L.; Vigna, S. Computationally Easy, Spectrally Good
        # Multipliers for Congruential Pseudorandom Number Generators. Softw
        # Pract Exp 2022, 52, 443–458, doi:10.1002/spe.3030.
        # Code is located at https://github.com/vigna/CPRNG.
        256: 92535799708728563004421432684894516311017097014017594320373447727772634342485,
    }

    self.output_size = output_size
    self.c = 1

    # Worst-case init (max state size), if the requested size is beyond
    # what we have in the table.
    self.a = ref_multipliers[256]
    # Look for a more suitable multiplier in our reference table.
    for state_size, multiplier in ref_multipliers.items():
      if state_size >= self.output_size * 2:
        self.a = multiplier
        break

  def RandomBits(self, n: int, *, seed: Optional[int] = None) -> int:
    """See base class."""
    output_size_bits = self.output_size
    output_size_bytes = (output_size_bits + 7) // 8
    state_size_bits = output_size_bits * 2
    state_size_bytes = (state_size_bits + 7) // 8

    req_size_bytes = (n + 7) // 8
    num_outputs = (req_size_bytes + output_size_bytes - 1) // output_size_bytes

    ba = bytearray(num_outputs * output_size_bytes)

    if seed is None:
      seed = int.from_bytes(os.urandom(state_size_bytes), "little")

    state = seed
    for j in range(num_outputs):
      state = (state * self.a + self.c) % 2**state_size_bits
      # Truncated LCG - output only the upper half of the state.
      output = state >> output_size_bits
      ba[output_size_bytes * j : output_size_bytes * (j + 1)] = output.to_bytes(
          output_size_bytes, "little"
      )
    if len(ba) != req_size_bytes:
      ba = ba[:req_size_bytes]
    if n % 8 != 0:
      ba[0] &= (1 << (n % 8)) - 1
...
class Mwc(Rng):
  ...
  def RandomBits(self, n: int, *, seed: Optional[int] = None) -> int:
    ...
    if seed is None:
      y = (
          int.from_bytes(os.urandom(self.ab1.bit_length() // 8 + 8), "little")
          % self.ab1
      )
    else:
      y = seed
...
class Lehmer(Rng):
  ...
  def RandomBits(self, n: int, *, seed: Optional[int] = None) -> int:
    ...
    if seed is None:
      while True:
        seed = (
            int.from_bytes(os.urandom(self.mod.bit_length() // 8 + 8), "little")
            % self.mod
        )
...
RNGS = {
    ...
    "mt19937": Mt19937(),
    "shake128": Shake128(),
    "trunclcg16": TruncLcgRand(16),
    "trunclcg20": TruncLcgRand(20),
    "trunclcg28": TruncLcgRand(28),
    "trunclcg32": TruncLcgRand(32),
    "trunclcg64": TruncLcgRand(64),
    "trunclcg128": TruncLcgRand(128),
